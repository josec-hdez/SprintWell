...
El resto de la memoria sigue el orden natural del problema: primero la revisión de la literatura, después la formalización matemática, luego el diseño y la implementación del sistema, la bitácora metodológica, la validación empírica y, finalmente, las conclusiones. Solo el capítulo 3 existe ya en versión consolidada en el momento de redactar esta introducción; el resto se redactará en orden conforme avance la implementación del MVP.
...
- **Capítulo 3 — Modelo formal del problema.** Ya formalizado en el capítulo 3 de esta memoria: conjuntos índice, parámetros, variables de decisión, las siete restricciones duras (R1–R7), la función de felicidad por usuario, los tres modos de equidad (utilitarista, max-min, Nash) y el análisis de NP-dificultad por reducción desde el _Generalized Assignment Problem_.
- **Capítulo 4 — Diseño del sistema.** Arquitectura en tres servicios independientes: un _backend_ NestJS con DDD en cuatro capas (presentation, application, domain, infrastructure), un _optimizer_ en Python con OR-Tools/CP-SAT, y un _frontend_ React. Decisiones de despliegue, contratos entre servicios, y la justificación de cada elección (brief §4).
- **Capítulo 5 — Implementación.** Detalles relevantes de la implementación: convenciones de código, _tooling_, estructura de paquetes, manejo de errores, persistencia, y los puntos donde la implementación se desvía del diseño teórico (si los hay) con su justificación.
- **Capítulo 6 — Metodología de desarrollo asistido por IA.** La bitácora del proceso: protocolo de uso de IA (cuándo se delega, cuándo se revisa, cuándo se rechaza la sugerencia), métricas de productividad, decisiones revertidas, _prompts_ representativos. Es el aporte metodológico declarado en §1.3.3.
- **Capítulo 7 — Validación.** _Benchmark_ sintético reproducible: CP-SAT frente a asignación aleatoria y _greedy_ por skill-match sobre datasets parametrizados (10–30 personas, 50–200 tareas). Calidad de las soluciones, tiempos de resolución, análisis cualitativo de las explicaciones generadas. Aquí se contrasta H2 con datos.
- **Capítulo 8 — Conclusiones y trabajo futuro.** Cierre: cuáles de H1, H2 y H3 quedaron validadas, cuáles refutadas y con qué evidencia. Líneas de trabajo futuro derivadas de las exclusiones explícitas de §1.4 y de los hallazgos no anticipados durante la validación.
