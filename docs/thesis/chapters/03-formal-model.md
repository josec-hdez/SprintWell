...
**Observación 3.1 (agregación absoluta vs. normalizada).** El modelo formal define la equidad sobre las felicidades **normalizadas** $f_j = \sum_k w_k c_k / \sum_k w_k$ (ec. 3.8). La implementación de referencia, en cambio, agrega los **términos absolutos** $\tilde f_j = \sum_k w_k c_k$ (el numerador, sin dividir por el peso total del usuario) para mantener la función objetivo lineal en las variables de cumplimiento y evitar divisiones en CP-SAT. Ambas formulaciones coinciden cuando todos los usuarios tienen el mismo peso total $\sum_k w_k$ — garantizado por el presupuesto fijo de 100 puntos (brief §6.2) **si cada usuario lo agota**. Cuando los pesos totales difieren (un usuario declara menos reglas o con menor peso), "maximizar el peor $\tilde f_j$ absoluto" no equivale a "maximizar el peor $f_j$ normalizado", y `max-min`/`nash` pueden reordenar quién es el peor servido en términos del índice reportado. El capítulo 7 documenta empíricamente esta divergencia en el caso de estudio, y el capítulo 8 la recoge como limitación con su vía de corrección (normalizar los términos de equidad). Se declara aquí para que el modelo formal quede fiel al código final.
